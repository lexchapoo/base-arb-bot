...
/// Upgradeable (UUPS) variant of BaseArbExecutor.
///
/// The execution, validation and accounting logic is byte-identical to the immutable
/// BaseArbExecutor. Only the initialization surface differs: `POOL` moves from an immutable
/// (which lives in implementation bytecode and would be lost across an upgrade) into storage,
/// and the constructor becomes an initializer that runs once against the proxy.
///
/// UPGRADE AUTHORITY IS A TOTAL-LOSS KEY. `_authorizeUpgrade` is `onlyOwner`, so whoever holds
/// the owner key can replace this logic with an implementation that sweeps every token the
/// proxy holds or is approved for. It is additionally `onlyIdle`, so an upgrade can never land
/// while a flash loan is in flight.
contract BaseArbExecutorUpgradeable is IFlashLoanSimpleReceiver, IMorphoFlashLoanCallback, Initializable, UUPSUpgradeable {
    error Unauthorized();
    error InvalidRoute();
    error StaleRoute();
    ...
    /// The implementation itself must never be initializable -- an attacker who initializes a
    /// bare implementation owns it, and under UUPS can call upgradeToAndCall on it directly.
    constructor() {
        _disableInitializers();
    }

    function initialize(address pool, address morpho) external initializer {
    ...
    /// Gates who may replace this contract's logic. `onlyIdle` additionally forbids upgrading
    /// while a route or batch is mid-flight, so an upgrade cannot swap validation rules out
    /// from under a flash loan that was authorised under the previous implementation.
    function _authorizeUpgrade(address) internal override onlyOwner onlyIdle {}

    /// The logic contract this proxy currently delegates to. Exposed so operators (and the
    /// fuzzing harness) can assert the implementation has not moved without relying on
    /// out-of-band eth_getStorageAt.
