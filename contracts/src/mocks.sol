...
contract MockPool is IAavePool {
    ...
    constructor(uint128 p) { premiumBps = p; }
    ...
    function flashLoanSimple(address receiver,address asset,uint256 amount,bytes calldata params,uint16) external {
        ...
        require(IFlashLoanSimpleReceiver(receiver).executeOperation(asset, amount, premium, receiver, params), "callback");
...
contract MockExchangeAdapter is IExchangeAdapter {
    ...
    function swap(address tokenIn,address tokenOut,uint256 amountIn,uint256 minOut,address recipient,bytes calldata) external returns (uint256 amountOut) {
