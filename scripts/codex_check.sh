...
python3 -m compileall -q python/src python/tests signer-gateway/src signer-gateway/tests scripts_prepare_deployment.py scripts_deploy_via_signer.py
if [ -x python/.venv/bin/python ]; then
  python/.venv/bin/python -m pytest -q python/tests
  PYTHONPATH=signer-gateway/src python/.venv/bin/python -m pytest -q signer-gateway/tests
else
  python3 -m pytest -q python/tests
  PYTHONPATH=signer-gateway/src python3 -m pytest -q signer-gateway/tests
fi
